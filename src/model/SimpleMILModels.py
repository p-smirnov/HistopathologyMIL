...
class TransformerMIL(L.LightningModule):
    def __init__(self, input_size, hidden_dim, n_heads, lr=0.0001, weight_decay=50e-4, mlp_dropout = 0.2, mha_dropout = 0.2, class_weights=Tensor([1.0]), position_encoding=False):
        super().__init__()
        ...
        self.save_hyperparameters(ignore=['position_encoding'])
        self.model = Transformer(self.position_encoding, self.transformer_config)

    def forward(self, x, **kwargs):
        mask = kwargs.get('mask', None)
        coords = kwargs.get('coords', None)
        Y_prob = self.model(x, mask = mask, coords = coords).squeeze()
    ...
    def training_step(self, batch, batch_idx):
        # training_step defines the train loop.
        # it is independent of forward
        x, y, mask = batch ## TODO: make this compatible with coordinates!!
        y = y.float()
    ...
    def validation_step(self, batch, batch_idx):
        x, y, mask = batch ## TODO: make this comparible with coordinates!!
        y = y.float()
        y_prob, y_hat = self.forward(x)
        # y_prob = torch.clamp(y_prob, min=1e-5, max=1. - 1e-5)
        loss = self.loss(y_prob,y)
